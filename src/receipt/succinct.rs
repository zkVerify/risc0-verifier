    ...
    Claim: Digestible + Debug + Clone + Serialize,
{
    ...
    pub fn verify_integrity_with_context(
        ...
        // reliable.
        ctx.verify_succinct(self.hashfn.as_str(), &self.seal, &self.control_inclusion_proof, &params)?;
        // risc0_zkp_v1::verify::verify(ctx.recursive_circuit, suite, &self.seal, check_code)?;

        // Extract the globals from the seal
        let output_elems: &[BabyBearElem] =
            bytemuck::checked::cast_slice(&self.seal);
        let mut seal_claim = VecDeque::new();
        for elem in output_elems.into_iter().take(ctx.succinct_output_size()) {
