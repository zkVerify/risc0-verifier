...
use crate::{circuit, MaybePruned, Proof, ReceiptClaim, Verifier};
use crate::circuit::CircuitCoreDefV2;
use super::{VerifierParameters, VC};
...
impl<SC: CircuitCoreDefV2, RC: CircuitCoreDefV2> VC for V2<SC, RC> {
    ...
    fn boxed_clone(&self) -> Box<dyn VC<Segment=Self::Segment, Succinct=Self::Succinct, HashSuite=Self::HashSuite>> {
        let cloned = Self {
    ...
    fn boxed_succinct_verifier_with_control_root(&self, control_root: Digest) -> Box<dyn VC<Segment=Self::Segment, Succinct=Self::Succinct, HashSuite=Self::HashSuite>> {
        alloc::boxed::Box::new(V2::empty(&circuit::v2_0::CIRCUIT, &circuit::v2_0::recursive::CIRCUIT)
    ...
    fn verify_segment(&self, hashfn: &str, seal: &[u32], _params: &SegmentReceiptVerifierParameters) -> Result<(), VerificationError> {
        let suite = self.verifier_parameters().suite(hashfn)
        ...
        risc0_zkp_v2::verify::verify(&CircuitImpl, suite, seal, check_code_fn).map_err(Translate::translate)
    }
    ...
    fn verify_succinct(&self, hashfn: &str, seal: &[u32], control_inclusion_proof: &MerkleProof, params: &SuccinctReceiptVerifierParameters) -> Result<(), VerificationError> {
        ...
        // reliable.
        risc0_zkp_v2::verify::verify(&circuit::v2_0::recursive::CIRCUIT, suite, seal, check_code).map_err(Translate::translate)
    }
    ...
    fn set_poseidon2_mix_impl(&mut self, poseidon2: Box<dyn Poseidon2Mix + Send + Sync + 'static>) {
        self.mut_verifier_parameters().suites
...
impl crate::context::CircuitInfo for SuccinctV2 {
    fn protocol(&self) -> ProtocolInfo {
        <circuit::v2_0::recursive::CircuitImpl as risc0_zkp_v2::adapter::CircuitInfo>::CIRCUIT_INFO.translate()
    }
    fn size(&self) -> usize {
...
impl<SC: CircuitCoreDefV2, RC: CircuitCoreDefV2> V2<SC, RC> {
    ...
    pub fn boxed(self) -> Box<dyn Verifier<HashSuite=HashSuiteV2>> {
        Box::new(self)
    }
}
