...
use crate::{circuit, CircuitCoreDefV1, MaybePruned, ReceiptClaim, Verifier};
...
use risc0_zkp_v1::adapter::PROOF_SYSTEM_INFO;
use crate::context::{SegmentV1, SuccinctV1, VerifierParameters, VerifierParametersV1, VC};
...
pub fn decode_receipt_claim_from_seal(
    seal: &[u32],
...
impl<SC: CircuitCoreDefV1, RC: CircuitCoreDefV1> VC for V1<SC, RC> {
    ...
    fn boxed_succinct_verifier_with_control_root(&self, control_root: Digest) -> alloc::boxed::Box<dyn VC<Segment=Self::Segment, Succinct=Self::Succinct, HashSuite=Self::HashSuite>> {
        alloc::boxed::Box::new(V1::empty(self.circuit, self.recursive_circuit)
...
impl<SC: CircuitCoreDefV1, RC: CircuitCoreDefV1> V1<SC, RC> {
    ...
    pub fn boxed(self) -> Box<dyn Verifier<HashSuite=HashSuite<BabyBear>>> {
        Box::new(self)
    }
}